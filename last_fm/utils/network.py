...
def get_network(*args, **kwargs):
    return pylast.LastFMNetwork(app.config["LAST_FM_API_KEY"], app.config["LAST_FM_API_SECRET"], *args, **kwargs)


def get_user_data(authenticated_user):
    data = {}
    for k in authenticated_user._request(
            "user.getInfo", True, {"user": authenticated_user.get_name()}).getElementsByTagName('*'):
